...
internal class DataVerifyingMessageAdapter<TMessage> : IMessageAdapter<TMessage>
{
	...
	public async Task HandleMessageAsync(RtgsMessage rtgsMessage, IHandler<TMessage> handler)
	{
		...
		if (_options.UseMessageSigning)
		{
			...
			if (!rtgsMessage.Headers.TryGetValue("pairwise-did-signature", out var privateSignature) || string.IsNullOrEmpty(privateSignature))
			{
			...
			if (!rtgsMessage.Headers.TryGetValue("from-rtgs-global-id", out var fromRtgsGlobalId) || string.IsNullOrEmpty(fromRtgsGlobalId))
			{
			...
			if (!verified)
			{
				var exception = new VerificationFailedException($"Verification of {MessageIdentifier} message failed.", MessageIdentifier);
				throw exception;
			}
