...
internal sealed class RtgsPublisher : IRtgsPublisher
{
	...
	public Task<SendResult> SendPayawayCreateAsync(PayawayCreationV1 message, string partnerRtgsGlobalId, CancellationToken cancellationToken = default) =>
		_internalPublisher.SendMessageAsync(message, cancellationToken, partnerRtgsGlobalId: partnerRtgsGlobalId);

	public Task<SendResult> SendPayawayConfirmationAsync(PayawayConfirmationV1 message, string partnerRtgsGlobalId, CancellationToken cancellationToken = default) =>
		_internalPublisher.SendMessageAsync(message, cancellationToken, partnerRtgsGlobalId: partnerRtgsGlobalId);

	public Task<SendResult> SendPayawayRejectionAsync(PayawayRejectionV1 message, string partnerRtgsGlobalId, CancellationToken cancellationToken = default) =>
		_internalPublisher.SendMessageAsync(message, cancellationToken, partnerRtgsGlobalId: partnerRtgsGlobalId);
